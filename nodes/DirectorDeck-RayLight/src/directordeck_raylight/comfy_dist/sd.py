...
from directordeck_raylight import comfy_dist
from directordeck_raylight.diffusion_models.wan.pipefusion import (
...
from directordeck_raylight.distributed_worker.pipefusion_schema import build_stage_plan
...
class OffsetBypassAdapter(comfy.weight_adapter.WeightAdapterBase):
    ...
    def __init__(self, entries, *, fail_on_mismatch=False):
    ...
    def _handle_shape_mismatch(self, base_key, offset, delta_shape, base_shape):
        if self.fail_on_mismatch:
            raise RuntimeError(
                "Raylight MiniMax H3 LoRA bypass output contract mismatch: "
                f"key={base_key}, offset={offset}, delta={tuple(delta_shape)}, "
                f"base={tuple(base_shape)}"
            )
            ...
            if offset is None:
                if delta.shape == base_out.shape:
                    ...
                    self._handle_shape_mismatch(
                        base_key, offset, delta.shape, base_out.shape
                    )
                continue
...
def _strict_h3_targets(model, lora, strength_model):
    targets = minimax_h3_lora_targets(model, lora) if model is not None else None
    if targets is not None and (
        not math.isfinite(float(strength_model)) or float(strength_model) == 0.0
    ):
        raise ValueError("Raylight MiniMax H3 LoRA strength must be finite and non-zero")
    return None if targets is None else set(targets)


def _require_exact_h3_keys(*, stage, expected, actual):
    if expected is None:
        return
    actual = set(actual)
    if actual != expected:
        raise RuntimeError(
            f"Raylight MiniMax H3 LoRA {stage} was incomplete: "
            f"missing={_sample_keys(sorted(expected.difference(actual), key=str))}, "
            f"unexpected={_sample_keys(sorted(actual.difference(expected), key=str))}"
        )


def _require_all_h3_source_keys_consumed(*, expected_targets, lora, loaded):
    if expected_targets is None:
        return
    consumed = set()
    for target, adapter in loaded.items():
        adapter_keys = getattr(adapter, "loaded_keys", None)
        if adapter_keys is None:
            raise RuntimeError(
                "Raylight MiniMax H3 LoRA adapter has no source-key evidence: "
                f"target={target}, adapter={type(adapter).__name__}"
            )
        consumed.update(adapter_keys)
    expected_keys = set(lora)
    if consumed != expected_keys:
        raise RuntimeError(
            "Raylight MiniMax H3 LoRA did not consume every source key: "
            f"missing={_sample_keys(sorted(expected_keys.difference(consumed), key=str))}, "
            f"unexpected={_sample_keys(sorted(consumed.difference(expected_keys), key=str))}"
        )


def load_minimax_h3_lora_for_models_strict(model, lora, strength_model):
    """Comfy bypass-equivalent loader with exact H3 application evidence."""

    key_map = comfy.lora.model_lora_keys_unet(model.model, {})
    lora = comfy.lora_convert.convert_lora(lora)
    expected = _strict_h3_targets(model, lora, strength_model)
    if expected is None:
        raise TypeError("Strict MiniMax H3 LoRA loader requires a MiniMaxH3Model")

    loaded = comfy.lora.load_lora(lora, key_map, log_missing=False)
    _require_exact_h3_keys(stage="adapter resolution", expected=expected, actual=loaded)
    _require_all_h3_source_keys_consumed(
    ...
    applied = set()
    if regular_patches:
        patched = set(new_modelpatcher.add_patches(regular_patches, strength_model))
        _require_exact_h3_keys(
            stage="regular patch installation",
            expected=set(regular_patches),
            actual=patched,
        )
        applied.update(patched)

    manager = comfy.weight_adapter.BypassInjectionManager()
    model_keys = set(new_modelpatcher.model.state_dict())
    for key, adapter in bypass_patches.items():
        if key not in model_keys:
            raise RuntimeError(f"Raylight MiniMax H3 LoRA target is absent: {key}")
        manager.add_adapter(key, adapter, strength=strength_model)
    injections = manager.create_injections(new_modelpatcher.model)
    if manager.get_hook_count() != len(bypass_patches):
        raise RuntimeError(
            "Raylight MiniMax H3 LoRA bypass hook installation was incomplete: "
            f"expected={len(bypass_patches)}, actual={manager.get_hook_count()}"
        )
    applied.update(bypass_patches)
    _require_exact_h3_keys(stage="application", expected=expected, actual=applied)
    if bypass_patches:
...
def load_lora_for_models(model, lora, strength_model):
    ...
    lora = comfy.lora_convert.convert_lora(lora)
    expected_h3 = _strict_h3_targets(model, lora, strength_model)

    # == Change from comfy ==#
    loaded = comfy_dist.lora.load_lora(lora, key_map)
    _require_exact_h3_keys(
        stage="adapter resolution", expected=expected_h3, actual=loaded
    )
    _require_all_h3_source_keys_consumed(
        expected_targets=expected_h3,
        lora=lora,
        loaded=loaded,
    )
    if model is not None:
    ...
    k = set(k)
    _require_exact_h3_keys(stage="patch installation", expected=expected_h3, actual=k)
    for x in loaded:
        if x not in k:
...
def load_lora_for_models_quantized(model, lora, strength_model, dynamic_sidecar=False, fallback_to_patches=False):
    ...
    for loaded_key, patch_data in loaded.items():
        ...
        if patch_name in {"lora", "loha", "lokr"} and hasattr(patch_data, "h"):
            ...
            # MiniMax H3 calls TensorWise-INT8 MLP fc2 through
            # comfy.ops.linear_input_act(), which reads linear.weight directly.
            # A forward bypass hook on fc2 therefore never fires. Keep these
            # patches on ModelPatcher's weight-function path so cast_bias_weight
            # materializes the patched weight before the fused call.
            if is_minimax_h3_fused_int8_fc2(new_modelpatcher, module_key):
                fused_int8_fc2_patches[loaded_key] = patch_data
                continue
    ...
    fused_int8_fc2_loaded_keys = set()
    if fused_int8_fc2_patches:
        patched_keys = set(new_modelpatcher.add_patches(fused_int8_fc2_patches, strength_model))
        missing_patch_keys = set(fused_int8_fc2_patches).difference(patched_keys)
        if missing_patch_keys:
            raise RuntimeError(
                "Raylight could not install MiniMax H3 fused INT8 fc2 LoRA weight patches: "
                f"{_sample_keys(missing_patch_keys)}"
            )
        fused_int8_fc2_loaded_keys.update(
            key[0] if isinstance(key, tuple) else key for key in patched_keys
        )
    ...
    manager = comfy.weight_adapter.BypassInjectionManager()
    loaded_keys = set(fused_int8_fc2_loaded_keys)
    for key, entries in sidecar_groups.items():
        ...
        manager.add_adapter(
            ...
            OffsetBypassAdapter(
                wrapper_entries,
                # H3 is the Director-owned strict path.  Other RayLight models
                # retain the upstream warning + zero-delta compatibility.
                fail_on_mismatch=expected_h3 is not None,
            ),
            strength=1.0,
        )
    ...
    if expected_h3 is not None:
        if unsupported_keys:
            raise RuntimeError(
                "Raylight MiniMax H3 LoRA contains adapters unsupported by the "
                f"active quantized path: {_sample_keys(unsupported_keys)}"
            )
        if hook_count != len(sidecar_groups):
            raise RuntimeError(
                "Raylight MiniMax H3 LoRA bypass hook installation was incomplete: "
                f"expected={len(sidecar_groups)}, actual={hook_count}"
            )
        _require_exact_h3_keys(
            stage="quantized application",
            expected=expected_h3,
            actual=expected_h3.intersection(loaded_keys),
        )
    ...
    if fused_int8_fc2_patches:
        logging.warning(
            "[Raylight LoRA][weight-function] MiniMax H3 fused_int8_fc2=%d patched=%d",
            len(fused_int8_fc2_patches),
            len(fused_int8_fc2_loaded_keys),
        )
    if dora_keys:
        logging.warning(
    ...
    if len(loaded) > 0 and hook_count == 0 and not fallback_to_patches and not fused_int8_fc2_loaded_keys:
        if expected_h3 is not None:
            raise RuntimeError(
                f"Raylight MiniMax H3 LoRA {mode} created no bypass hook or weight patch"
            )
...
def gguf_load_diffusion_model(unet_path, model_options={}, dequant_dtype=None, patch_dtype=None):
    from directordeck_raylight.expansion.comfyui_gguf.ops import GGMLOps
    from directordeck_raylight.expansion.comfyui_gguf.loader import gguf_sd_loader
    from directordeck_raylight.expansion.comfyui_gguf.nodes import GGUFModelPatcher
