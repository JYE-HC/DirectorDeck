...
from directordeck_raylight.distributed_modules.pipefusion import (
...
from directordeck_raylight.distributed_modules.usp import USPInjectRegistry
from directordeck_raylight.distributed_modules.cfg import CFGParallelInjectRegistry
from directordeck_raylight.distributed_worker.pipefusion_schema import (
    PipeFusionConfig,
    build_stage_plan,
)
from directordeck_raylight.distributed_worker.pipefusion_state import (
    PIPEFUSION_RUNTIME_ATTACHMENT,
    PIPEFUSION_WRAPPER_KEY,
    PipeFusionRuntime,
)
from directordeck_raylight.distributed_worker.parallel_group_manager import (
    initialize_xfuser_parallel,
    requires_xfuser_parallel,
)
from directordeck_raylight.distributed_worker.ray_worker_controlnet import (
...
from directordeck_raylight.distributed_worker.utils import Noise_EmptyNoise, Noise_RandomNoise, patch_ray_tqdm
from directordeck_raylight.comfy_dist.quant_ops import patch_temp_fix_ck_ops
from directordeck_raylight.diffusion_models.minimax.lora import (
    is_minimax_h3_model_patcher,
...
# Per-worker LRU cap for base models offloaded to CPU RAM when the active
# model key changes. Weights are never dropped on a model switch: they move
# GPU -> CPU RAM and a later switch back activates from RAM without touching
# the checkpoint file. Can be overridden per workflow through the optional
# ``ram_cache_max_models`` input of RayInitializerAdvanced (0 disables the
# cache and frees VRAM directly, matching the legacy switch behaviour).
_RAM_CACHE_DEFAULT_MAX_BASE_MODELS = 2
...
def patch_enable_comfy_kitchen_fsdp(fn):
    ...
    def wrapper(self, *args, **kwargs):
        from directordeck_raylight.comfy_dist.kitchen_distributed import patch_enable_comfy_kitchen_fsdp as patcher
...
def _build_ray_guider(model, guider_spec):
    ...
    from directordeck_raylight.expansion.comfyui_ltxv.guiders import build_ltxv_ray_guider
...
class RayWorker:
    def __init__(self, local_rank, device_id, parallel_dict):
        ...
        # RAM cache of inactive base models (offloaded to CPU on switch),
        # keyed by (unet_path, model_options). LRU-capped per worker.
        self.ram_cache = OrderedDict()
        self.base_patcher = None
        ...
        print(
            f"[RayWorker {self.local_rank}] Ray exposes "
            f"{torch.cuda.device_count()} logical CUDA device(s); using {self.device}"
        )
        _enable_worker_dynamic_vram(worker_cli_args)
        _apply_worker_late_comfy_cli_args(worker_cli_args)

        from directordeck_raylight.comfy_dist import patch_base_getattr
    ...
    def set_meta_model(self, model):
        ...
        if first_param_device == torch.device("meta"):
            ...
            self.state_dict = None
            self.model = model
            self._require_director_minimax_h3_model()
    ...
    def check_model_loaded(self, unet_path, model_options):
        ...
        is_loaded = self.model is not None and self.active_request_key == active_key and not self._fsdp_init_failed()
        if is_loaded:
            self._require_director_minimax_h3_model()
        return is_loaded

    def _require_director_minimax_h3_model(self):
        """Fail closed when a Director Ray workflow loaded the wrong model family."""

        if self.parallel_dict.get("director_strict_h3") is not True:
            return
        if not is_minimax_h3_model_patcher(self.model):
            base_model = getattr(self.model, "model", None)
            diffusion_model = getattr(base_model, "diffusion_model", None)
            raise RuntimeError(
                "Director Ray workflows require an actual MiniMaxH3Model diffusion "
                f"model; got {type(diffusion_model).__name__}"
            )
    ...
    def _reset_active_model(self):
        ...
        if self.model is not None:
            if hasattr(self.model, "free_fsdp_vram"):
                ...
                # clone() shares the underlying model with ram_cache entries,
    ...
    def _ram_cache_limit(self):
        return int(
            self.parallel_dict.get("ram_cache_max_models", _RAM_CACHE_DEFAULT_MAX_BASE_MODELS)
        )

    def _evict_ram_cache(self):
        limit = self._ram_cache_limit()
        while len(self.ram_cache) > limit:
            _, oldest = self.ram_cache.popitem(last=False)
            try:
                import comfy.model_management as comfy_model_management

                comfy_model_management.unload_model_and_clones(
                    oldest, unload_additional_models=False
                )
            except Exception:
                pass
            del oldest
    ...
    def _offload_active_to_ram_cache(self):
        """Move the active model from GPU VRAM to CPU RAM and keep it cached.

        This is the model-switch path only. The same-model VRAM fast path in
        load_unet() is untouched: reusing the currently active model keeps its
        weights on the GPU exactly as before. Only when the requested key
        differs is the outgoing base unpatch'd to CPU RAM and LRU-cached, so
        switching back never re-reads the checkpoint file.
        """
        import comfy.model_management as comfy_model_management

        if self.model is None:
            return
        base_key = self.current_base_key
        moved_to_cpu = True
        try:
            self.model.unpatch_model(device_to=torch.device("cpu"))
        except Exception as e:
            moved_to_cpu = False
            print(f"[Rank {self.local_rank}] unpatch_model(CPU) failed in _offload_active_to_ram_cache: {e}")
        try:
            self.model.cleanup()
        except Exception as e:
            print(f"[Rank {self.local_rank}] model.cleanup() failed in _offload_active_to_ram_cache: {e}")

        # Cache the pristine base (no LoRA), never the LoRA-patched active
        # clone. Under AIMDO the active patcher is a ModelPatcherDynamic whose
        # unpatch_model keeps weight patches, so caching the active clone would
        # double-apply LoRA on the next activation.
        base = self.base_patcher
        if moved_to_cpu and base is not None and base_key is not None and self._ram_cache_limit() > 0:
            self.ram_cache[base_key] = base
            self.ram_cache.move_to_end(base_key)
            self._evict_ram_cache()
        self.model = None
        self.base_patcher = None
        self.current_base_key = None
        self.overwrite_cast_dtype = None
    ...
    def _activate_from_ram_cache(self, base_key, active_key):
        self.base_patcher = self.ram_cache.pop(base_key)
        self.model = self.base_patcher.clone()
        self._require_director_minimax_h3_model()
    ...
    def load_unet(self, unet_path, model_options):
        if self.parallel_dict["is_fsdp"] is True:
            ...
            # Different key: offload the outgoing base to CPU RAM instead of
            # dropping it, then prefer the RAM cache over a disk reload.
            self._offload_active_to_ram_cache()

            if base_key in self.ram_cache:
                self._activate_from_ram_cache(base_key, active_key)
                return

            self._free_cached_aux_models()
            if self.parallel_dict.get("pipefusion_enabled"):
                from directordeck_raylight.comfy_dist.sd import pipefusion_load_diffusion_model
                ...
                from directordeck_raylight.comfy_dist.sd import lazy_load_diffusion_model
            ...
            self.base_patcher = loaded_model
            self.current_base_key = base_key
            self.model = loaded_model.clone()
            self._require_director_minimax_h3_model()
    ...
    def load_gguf_unet(self, unet_path, dequant_dtype, patch_dtype, use_mmap=None):
        ...
        if self.parallel_dict["is_fsdp"] is True:
            ...
            from directordeck_raylight.comfy_dist.sd import gguf_load_diffusion_model
    ...
    def load_lora(
        ...
        for lora in self.lora_list:
            ...
            lora_model, normalized_key_count = normalize_minimax_h3_lora_keys(self.model, lora_model)
            if normalized_key_count:
                logging.warning(
                    "[Raylight LoRA][MiniMax H3] normalized %d bare keys to diffusion_model.*",
                    normalized_key_count,
                )

            if self.parallel_dict["is_fsdp"] is True:
                from directordeck_raylight.comfy_dist.sd import (
                ...
                if is_minimax_h3_model_patcher(self.model):
                    from directordeck_raylight.comfy_dist.sd import (
                        load_minimax_h3_lora_for_models_strict,
                    )

                    self.model = load_minimax_h3_lora_for_models_strict(
                        self.model,
                        lora_model,
                        strength_model,
                    )
                else:
                    self.model = comfy_sd.load_lora_for_models(
...
class RayCOMMTester:
    def __init__(self, local_rank, world_size, device_id):
        ...
        print(
            f"[RayTest {local_rank}] Ray exposes "
            f"{torch.cuda.device_count()} logical CUDA device(s); using {device}"
        )
