...
from directordeck_raylight.distributed_modules.attention_backends import validate_attention_backend_config
...
def initialize_xfuser_parallel(local_rank: int, world_size: int, parallel_dict: dict) -> XFuserParallelContext:
    ...
    if parallel_dict.get("is_xdit"):
        validate_attention_backend_config(parallel_dict["attention"], base_config.ring_degree)
    ...
    if parallel_dict.get("is_xdit"):
        ...
        xfuser_attn.set_ring_degree(config.ring_degree)
