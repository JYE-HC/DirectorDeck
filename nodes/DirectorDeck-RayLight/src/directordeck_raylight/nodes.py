...
# The outer entrypoint adds this private package to sys.path for Ray workers.
...
from .driver_memory import (
    DRIVER_CLEANUP_POLICIES,
    build_driver_cleanup_metadata,
    cleanup_driver_models_for_ray,
    clear_ray_worker_vram_after_sampling,
)
...
def _director_runtime_metadata(ray_cluster_namespace: str) -> dict[str, bool]:
    """Derive Director-only worker policy without changing the prompt schema."""

    return {
        "director_strict_h3": isinstance(ray_cluster_namespace, str)
        and ray_cluster_namespace.startswith("director-")
    }
...
_RAYLIGHT_MODULE_PATH = _resolve_module_dir(directordeck_raylight)
...
class RayInitializer:
        ...
        return {
            ...
            "optional": {
                "driver_cleanup_policy": (
                    list(DRIVER_CLEANUP_POLICIES),
                    {
                        "default": "legacy_all",
                        "tooltip": "legacy_all clears every ComfyUI driver model before Ray sampling. ray_devices clears the local driver GPU selection exposed to Ray.",
                    },
                ),
            },
        }
    ...
    def spawn_actor(
        ...
        driver_cleanup_policy: str = "legacy_all",
        ram_cache_max_models: int = 2,
    ):
        ...
        validate_attention_backend_config(XFuser_attention, effective_ring_degree)

        selected_gpus = _parse_gpu_select(GPU_SELECT)
        ray_cluster_is_local = ray_cluster_address in _LOCAL_CLUSTER_ADDRESSES
        ...
        self.parallel_dict["ram_cache_max_models"] = ram_cache_max_models
        self.parallel_dict.update(_director_runtime_metadata(ray_cluster_namespace))
        self.parallel_dict.update(
            build_driver_cleanup_metadata(
                driver_cleanup_policy,
                ray_cluster_is_local,
                selected_gpus,
        ...
        if selected_gpus is not None:
            ...
            # GPU_SELECT is expressed in the driver's logical CUDA namespace.
            # For a local Ray cluster, map those indices back through ComfyUI's
            # parent mask (for example 0,1 through 7,6 -> 7,6). A remote
            # cluster has its own physical namespace, so preserve the previous
            # direct-index behavior there.
            parent_cuda_visible_devices = (
                os.environ.get("CUDA_VISIBLE_DEVICES")
                if ray_cluster_address in _LOCAL_CLUSTER_ADDRESSES
                else None
            )
            restricted_cuda_visible_devices = resolve_cuda_visible_devices(
                selected_gpus,
                parent_cuda_visible_devices,
            ...
            runtime_env_base.setdefault("env_vars", {})["CUDA_VISIBLE_DEVICES"] = restricted_cuda_visible_devices
            print(
                "[Raylight] GPU_SELECT logical={} resolved {} Ray device(s); "
                "full CUDA mask omitted from normal logs".format(
                    ",".join(str(gpu_idx) for gpu_idx in selected_gpus),
                    len(restricted_cuda_visible_devices.split(",")),
                )
            )
        ...
        return {
            "required": {
                ...
                "GPU_SELECT": (
                    ...
                    {
                        "default": "",
                        "tooltip": "Logical GPU indices within the devices already visible to ComfyUI. For example, when ComfyUI starts with --cuda-device 7,6, use 0,1 here to select physical GPUs 7 and 6. Leave empty to inherit every visible GPU.",
                    },
                ),
                "ulysses_degree": (
                    "INT",
            ...
            "optional": {
                ...
                "ram_cache_max_models": (
                    "INT",
                    {
                        ...
                        "min": 0,
                        "max": 64,
                        "tooltip": "Per-worker LRU cap for base models offloaded to CPU RAM when the requested model key changes. Switching models moves the outgoing weights to CPU RAM and a switch back activates them from RAM without re-reading the checkpoint. 0 disables the cache and frees VRAM directly like the legacy switch path.",
                    },
                ),
            },
        }
...
class RayLoraLoader:
    ...
    def load_lora(self, lora_name, strength_model, prev_ray_lora=None):
        ...
        if not math.isfinite(float(strength_model)) or float(strength_model) == 0.0:
            raise ValueError("RayLoraLoader strength_model must be finite and non-zero")
...
class XFuserKSamplerAdvanced:
    ...
    def ray_sample(
        ...
        cleanup_driver_models_for_ray(ray_actors)
        ...
        clear_ray_worker_vram_after_sampling(ray_actors)
...
class UnifiedParallelSampler:
    ...
    def ray_sample(
        ...
        cleanup_driver_models_for_ray(ray_actors, parallel_dict)
        futures = [
            actor.common_ksampler.remote(
